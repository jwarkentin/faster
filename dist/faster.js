/**
 * Faster.js Javascript performance library v0.0.1
 *
...
(function() {
    (function() {
        ...
        function regexpEscape(str) {
            return str.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\//@platform-detect");
        }
    ...
    function stringifyMap(map) {
        ...
        for (var prop in map) {
            if (map.hasOwnProperty(prop)) {
                if (map[prop] instanceof Function) newMap[prop] = map[prop].toString(); else newMap[prop] = map[prop];
            }
        }
     ...
     */
        select: function select(obj, options) {
            ...
            options = extend({
                platform: platform,
                // By default select based on the current platform as determined by platform-detect.
                // This is mostly only useful for unit testing.
                test: false,
                // Test for the fastest version instead of using the defined map
                forceTest: false,
                // If 'test' is true, but we have cached a function mapKey for the given map object,
...
(function() {
    var ObjectKeys = {
        ...
        map: {
            "default": function() {
                if (Object.keys) return "keys1"; else return "keys2";
            },
            keys1: [],
            keys2: []
        },
